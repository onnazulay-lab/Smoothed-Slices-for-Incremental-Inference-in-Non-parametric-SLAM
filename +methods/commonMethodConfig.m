...
arguments
    ...
    % "rcsGeneral" is the SAME estimator as "rcs" at the depth the app runs,
    % routed through methods.smoothed.surfaceRecursionGeneral instead of the
    % scalar unrolling. It is a developer route, not a third method: it exists
    % so the general recursion the depth study relies on is exercised against
    % an implementation a quadrature reference has already validated. There is
    % deliberately no GUI control for it -- see research.recursionDepthStudy.
    opts.innerEstimator (1,1) string {mustBeMember(opts.innerEstimator, ["nested","rcs","rcsGeneral"])} = "nested"
